...
class Example2ControllerTest {
  ...
  public void shouldSaveAndReturnTheSameGeneratedExampleForGetByIdEndpoint()
    ...
    // THEN
    response = given().when().get("/examples2/{id}", uuid).then().statusCode(200).extract();
    System.out.println("Response payload:");
    System.out.println(response.asPrettyString());
    JSONAssert.assertEquals(json, response.asString(), true);
  }
  ...
  //    @Test
  //    public void shouldSaveAndPatchModelWithJsonPatchSpecification() throws IOException,
  // JSONException {
  //        // GIVEN
  //        UUID uuid = UUID.randomUUID();
  //        Example2Dto dto = generator.nextObject(Example2Dto.class);
