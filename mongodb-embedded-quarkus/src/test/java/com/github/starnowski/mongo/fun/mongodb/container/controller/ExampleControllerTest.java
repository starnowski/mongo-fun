...
class ExampleControllerTest {
    ...
    public static Stream<Arguments> provideShouldSaveExampleDocumentWithParameters() {
        return Stream.of(
                Arguments.of("examples/example2_withQueryParams.json", Map.of("names", Arrays.asList("Simon", "Doug")), "examples/example2_withQueryParams.json")
        );
    }
    ...
    public void shouldSaveExampleDocumentWithQueryParams(String requestFile, Map queryParams, String expectedResponse) throws IOException, JSONException {
        ExtractableResponse<Response> response = given()
                .body(Files.readString(Paths.get(new File(getClass().getClassLoader().getResource(requestFile).getFile()).getPath())))
                .contentType(ContentType.JSON)
