...
public class ExampleDao extends AbstractDao<Document> {
  ...
  @Inject private Example2StaticEdmSupplier example2StaticEdmSupplier;
  ...
  public List<Document> query(List<String> filters, List<String> orders, List<String> select)
    ...
    return pipeline.isEmpty()
        ? new ArrayList<>()
        : getCollection().aggregate(pipeline).into(new ArrayList<>());
  }

  public String explain(List<String> filters, List<String> orders, List<String> select)
      throws ExpressionVisitException,
          ODataApplicationException,
          UriValidationException,
          UriParserException {
    ...
    System.out.println(
        new Document("pipeline", pipeline)
            .toJson(getCollection().getCodecRegistry().get(Document.class)));
  ...
  private List<Bson> preparePipelineBasedOnFilter(
      List<String> filters, List<String> orders, List<String> select)
    ...
    if (filters != null
      ...
      UriInfo uriInfo =
          new Parser(example2StaticEdmSupplier.get(), OData.newInstance())
              .parseUri(
                  "examples2",
      ...
      if (filterOption != null) {
        ...
        try {
          ...
          System.out.println(
              "<pipeline>"
                  + Aggregates.match(Filters.and(bsonFilter))
          ...
          String path =
              Paths.get(
                      new File(getClass().getClassLoader().getResource("/testcases.txt").getFile())
                          .getPath())
                  .toAbsolutePath()
                  .toString();
    ...
    if (orders != null
      ...
      if (orderOption != null) {
        Bson bsonFilter =
            OdataOrderToMongoSortParser.parseOrder(uriInfo, example2StaticEdmSupplier.get());
