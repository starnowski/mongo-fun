...
public class OpenApiExample2ValidationFilter implements ContainerRequestFilter {
  ...
  @Inject private OpenApiJsonValidator openApiJsonValidator;
  ...
  public void filter(ContainerRequestContext requestContext) throws IOException {
    ...
    // Log the request
    System.out.printf(
        "Incoming %s request to %s%n",
        requestContext.getMethod(), requestContext.getUriInfo().getRequestUri());

    Set<ValidationMessage> errors =
        this.openApiJsonValidator.validateObjectSpec2("Example2", getRequestBody(requestContext));
    if (errors.isEmpty()) {
      ...
      errors.forEach(
          err -> {
            ...
            error.append(
                "location", err.getInstanceLocation().toString().replace("$.", "request."));
            errorsList.add(error);
          });
      document.append("errors", errorsList);
      requestContext.abortWith(
          Response.status(Response.Status.BAD_REQUEST).entity(document.toJson()).build());
    }
  }
