...
public class OpenApiJsonMapper {
  ...
  public OpenApiJsonMapper() {
    ...
    mongoDBModule.addSerializer(
        Binary.class,
        new JsonSerializer<Binary>() {
          @Override
  ...
  public Map<String, Object> coerceRawJsonTypesToOpenApiJavaTypes(
      Map<String, Object> jsonMap, String openApiSpec, String schemaRef) throws Exception {
    return parse(
        mapper.valueToTree(jsonMap),
        openApiSpec,
        schemaRef,
        (value, type, format) -> coerceWithSchema(value, type, format));
  }
  ...
  //            String schemaRef
  //    ) throws Exception {
  //        return parse(mapper.valueToTree(jsonMap), openApiSpec, schemaRef, (value, type, format)
  // -> coerceJavaTypeWithSchema(value, type, format));
  //    }
  ...
  public Map<String, Object> coerceMongoDecodedTypesToOpenApiJavaTypesV2(
    ...
    return (Map<String, Object>)
        coerceDeepWithSchema(
            jsonMap,
            oasSchema,
            ((value, type, format) ->
                coerceJavaTypeWithSchemaAndMappedByMapper(value, type, format)));
  }

  @SuppressWarnings("unchecked")
  private Object coerceDeepWithSchema(
      Object value, Schema<?> schema, CoerceValueWithSchema coerceValueWithSchema) {
    ...
    if (value instanceof Map<?, ?> subMap && schema.getProperties() != null) {
      ...
      for (Object item : list) {
        newList.add(
            coerceDeepWithSchema(item, (Schema<?>) schema.getItems(), coerceValueWithSchema));
      }
      return newList;
    } else {
  ...
  private Object coerceJavaTypeWithSchemaAndMappedByMapper(
      Object value, String type, String format) {
    if (value == null) return null;
