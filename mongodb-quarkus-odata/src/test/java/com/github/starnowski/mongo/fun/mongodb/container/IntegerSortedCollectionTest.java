...
public class IntegerSortedCollectionTest {
  ...
  private static Stream<Arguments>
      provide_shouldReturnCorrectCountNumberAfterSeveralStagesWithDifferentCursorOperations() {
    return Stream.of(
        Arguments.of(
            asList(Aggregates.limit(600), Aggregates.skip(170), Aggregates.skip(200)),
            230,
            "limit 600, skip 170, skip 200"),
        Arguments.of(
            asList(Aggregates.skip(300), Aggregates.limit(400), Aggregates.skip(200)),
            200,
            "skip 300, limit 400, skip 200"));
  }

  private static Stream<Arguments>
      provide_shouldReturnCorrectFirstAndLastResultAfterSeveralStagesWithDifferentCursorOperations() {
    return Stream.of(
        Arguments.of(
            asList(Aggregates.limit(600), Aggregates.skip(170), Aggregates.skip(200)),
            370,
        ...
        Arguments.of(
            asList(
                Aggregates.limit(600),
                Aggregates.skip(170),
                Aggregates.skip(200),
                Aggregates.sort(Sorts.descending(INTEGER_COLUMN))),
            599,
            370,
            "limit 600, skip 170, skip 200, sort ascending"),
        Arguments.of(
            asList(
                Aggregates.limit(600),
                Aggregates.skip(170),
            ...
            399,
            370,
            "limit 600, skip 170, skip 200, sort ascending, skip 200"));
  }
  ...
  public void setUp() {
    sortable =
        mongoClient
            .getDatabase("test")
            .getCollection("integerSortedCollection")
  ...
  @ParameterizedTest(
      name = "should return expected count number {1} after several stage operations : {2}")
  @MethodSource(
      "provide_shouldReturnCorrectCountNumberAfterSeveralStagesWithDifferentCursorOperations")
  public void shouldReturnCorrectCountNumberAfterSeveralStagesWithDifferentCursorOperations(
      List<Bson> stages, int expectedCount, String message) {
    // GIVEN
    List<Document> projection = new ArrayList<>();
    List<Bson> pipeline = new ArrayList<>();
  ...
  @ParameterizedTest(
      name =
          "should return expected first number {1} and last number {2} after several stage operations : {3}")
  @MethodSource(
      "provide_shouldReturnCorrectFirstAndLastResultAfterSeveralStagesWithDifferentCursorOperations")
  public void shouldReturnCorrectFirstAndLastResultAfterSeveralStagesWithDifferentCursorOperations(
      List<Bson> pipeline, int first, int last, String message) {
    // GIVEN
