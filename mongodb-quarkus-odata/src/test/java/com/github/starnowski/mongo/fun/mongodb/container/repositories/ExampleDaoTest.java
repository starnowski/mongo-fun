...
class ExampleDaoTest {
  ...
  public static Stream<Arguments> provideTestPipelineThatFails() {
    return Stream.of(
        ...
        Arguments.of(
            Aggregates.match(
                Filters.elemMatch(
                    "tags", Filters.and(new Document("$regex", "x").append("$options", "i")))),
            0),
        Arguments.of(
            Aggregates.match(
                Filters.elemMatch(
                    "tags", Filters.or(new Document("$regex", "x").append("$options", "i")))),
            0),
        Arguments.of(
            Aggregates.match(Filters.elemMatch("tags", Filters.and(new Document("$eq", "x")))), 0),
  ...
  public static Stream<Arguments> provideTestPipelineThatWorks() {
    return Stream.of(
        Arguments.of(
            Aggregates.match(
                Filters.elemMatch("tags", new Document("$regex", "x").append("$options", "i"))),
            1),
        Arguments.of(
            Aggregates.match(
                Filters.elemMatch(
  ...
  @ParameterizedTest
  @MethodSource({"provideTestPipelineThatFails"})
  @MongoSetup(
      mongoDocuments = {
  ...
  @ParameterizedTest
  @MethodSource({"provideTestPipelineThatWorks"})
  @MongoSetup(
      mongoDocuments = {
        @MongoDocument(bsonFilePath = "examples/query/example2_1.json", collection = "examples"),
        @MongoDocument(bsonFilePath = "examples/query/example2_2.json", collection = "examples"),
